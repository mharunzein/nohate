    ...
    if request.method == 'GET':
        ...
        def process_lr(text):
            ...
            if pred_lr[0] == 0:
                ...
                return 'adalah ujaran kebencian'
        ...
        if request.form['predict'] =='' and request.files['file'].filename =='':
            return render_template('index.html', pesan='Harap masukkan kalimat atau file!!')

        elif request.form['predict'] !='' and request.files['file'].filename !='':
            return render_template('index.html', pesan='Harap pilih salah satu')
            ...
            data['HasilProcess_lr'] = data['HasilPreprocess'].apply(process_lr)
            data['HasilProcess_svm'] = data['HasilPreprocess'].apply(process_svm)
            data['HasilProcess_dt'] = data['HasilPreprocess'].apply(process_dt)
            data['HasilProcess_nb'] = data['HasilPreprocess'].apply(process_nb)
            data['HasilProcess_stacking'] = data['HasilPreprocess'].apply(process_stacking)
...
def visualisasi():
    if request.method == 'GET':
        ...
        if request.form['dropdown'] =='1':
            return render_template('visualisasi.html', model = "Linear Regression", confussion = cm_lr, akurasi = acc_lr, recall = rec_lr, f1_score = f1_lr)
        elif request.form['dropdown']=='2':
            return render_template('visualisasi.html', model = "Support Vector Machine", confussion = cm_svm, akurasi = acc_svm, recall = rec_svm, f1_score = f1_svm)
        elif request.form['dropdown']=='3':
            return render_template('visualisasi.html', model = "Naive Bayes (Bernoulli)", confussion = cm_nb, akurasi = acc_nb, recall = rec_nb, f1_score = f1_nb)
        elif request.form['dropdown']=='4':
            return render_template('visualisasi.html', model = "Decision Tree", confussion = cm_dt, akurasi = acc_dt, recall = rec_dt, f1_score = f1_dt)
        elif request.form['dropdown']=='5':
            return render_template('visualisasi.html', model = "Stacking", confussion = cm_stacking, akurasi = acc_stacking, recall = rec_stacking, f1_score = f1_stacking)
    else:
        return "Unsupported Request Method"
